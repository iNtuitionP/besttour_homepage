 ...
 *
 * 0003_consent.sql 의 컬럼 4개 값을 만든다. DB·네트워크·서버 액션 없음 — 접수 서버 액션(P3)이 이 결과를
...
/**
 * 개인정보 처리방침 문안 버전. 방침 문안(P1-6 /privacy)이 바뀔 때마다 그 날짜로 올린다 —
 * 행마다 "동의 당시 버전"이 남아, 나중에 문안이 바뀌어도 그때 무엇에 동의했는지 말할 수 있다.
 */
...
export type ConsentInput = Pick<ReservationInput, "privacyConsent" | "marketingConsent">;

/**
 * 0003 컬럼 4개. `now` 는 서버가 접수 요청을 받은 인스턴트 — 동의 시각이자 retention_until 의 기산점이다.
 * DB 의 created_at(default now())과는 ms 단위로만 어긋나며, 0003 제약의 허용 폭(+5분 / -1일)이 그 오차를 흡수한다.
 */

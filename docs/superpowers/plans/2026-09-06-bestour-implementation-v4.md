...
| ID | 태스크 | 파일 | 의존 | 검증 |
|---|---|---|---|---|
| P6-1 | 갤러리 아키텍처 확정(컨트롤러) → `0008_gallery_albums.sql`(앨범·width/height/bytes/original_path) | `supabase/migrations/0008_*.sql` | P1-2 | 테스트 선작성 |
...
- [ ] **Supabase 인증 설정 3건**(`docs/ops/known-defects.md` D2): 공개 가입 차단 ✅(2026-09-13 조치) · **커스텀 SMTP**(기본 메일은 시간당 2통이고 발송 실패가 성공 화면으로 삼켜진다) · `site_url`·`uri_allow_list` 를 운영 도메인으로
- [ ] 방어 4종 실키 스모크 통과
...
- [ ] **`OWNER_PHONE` 설정** — 비어 있으면 `planNotifications`(outbox.ts:99-108)가 사장님 행을 `channel='email'` 로 만들고, P4-5 미구현이라 어댑터가 거부한다. 즉 **접수가 들어와도 사장님께 아무 통지가 가지 않는다**(고객 문자는 정상). 사장님 질문 A-13

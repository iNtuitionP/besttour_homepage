...
- **도구 출력의 "모양" 에 단언을 걸지 마라 — 값을 뽑아서 걸어라** (P5-15, 2026-09-19). `supabase db query` 는 CLI 버전마다 형식이 다르다: 로컬 2.117.0 은 JSON(값에 따옴표), CI 의 `setup-cli@v1 version: latest` 는 ASCII 표(`│ 값 │`, 따옴표 없음). 값 끝의 `"` 에 기댄 정규식 두 개가 **로컬 전량 초록 · CI DB Smoke 만 실패**(`f696020`)로 나타났다. `tests/helpers/local-stack-sql.ts` 의 `sqlCells()`·`sqlErrorText()` 로 형식을 지운 뒤 단언한다(JSON·표·csv 세 형식 실측). 메타 검사가 따옴표에 기댄 `toMatch` 를 다시 막는다.
  **CI 로그는 관리자 권한이 없어 API 로 못 받는다**(403). 대신 `GET /repos/:o/:r/commits/:sha/check-runs` → 실패 잡의 `check-runs/<id>/annotations` 에 실패 테스트 이름과 diff 가 그대로 들어 있다.

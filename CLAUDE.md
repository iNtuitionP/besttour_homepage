...
- **도메인 bestour.co.kr 사용종료일 = 2026-10-05.** 만료 시 홈페이지·회사 메일 동시 정지, 이후 제3자 선점 가능. 갱신처는 **(주)가비아**(호스팅사 카페24가 아님). 어떤 개발 태스크보다 우선한다.
- **회사 메일은 살아 있다** — MX `uws64-098.cafe24.com`. 카페24 웹호스팅 해지 = 메일 즉사. 옛 사이트 종료와 메일 이전은 분리 관리. 스펙 §13.4의 "메일 없음" 기술은 **정정됨**.
- DNS 현황: A `183.111.174.56`(카페24) · NS `ns1/ns2.cafe24.com` · SPF 없음 · DNSSEC 미서명. 전환은 **A 레코드만 Vercel로** 바꾸고 MX는 건드리지 않는다.
